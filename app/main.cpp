...
static td_bool g_app_exit = TD_FALSE;
...
#define APP_DMIC_GAIN 90 /* 90: 24dB */
...
static audio_app g_audio_apps[OT_AI_DEV_MAX_NUM * OT_AI_MAX_CHN_NUM];
...
void *app_file_proc(void *parg) {
  td_s32 ret;
  td_s32 ai_fd = -1;
  ot_audio_frame frame = {};
  ot_aec_frame aec_frm = {};
  audio_app *ai_ctrl = (audio_app *)parg;
  fd_set read_fds;
  ...
  while (ai_ctrl->start) {
    ret = app_audio_select(ai_fd, &read_fds);
    if (ret != TD_SUCCESS) {
      break;
    }
...
// create worker
td_s32 app_create_thread(ot_aio_attr *aio_attr, ot_audio_dev ai_dev,
                         ot_ai_chn ai_chn, FILE *res_fd) {
  audio_app *ai_to_ext_res = TD_NULL;
  ...
  ai_to_ext_res = &g_audio_apps[ai_dev * OT_AI_MAX_CHN_NUM + ai_chn];
  ...
  pthread_create(&ai_to_ext_res->ai_pid, 0, app_file_proc, ai_to_ext_res);

  return TD_SUCCESS;
}

// destroy worker
td_s32 app_destroy_thread(ot_audio_dev ai_dev, ot_ai_chn ai_chn) {
  audio_app *ai_to_ext_res = TD_NULL;
...
static td_void app_exit_proc(td_void) {
  td_u32 dev_id, chn_id;

  for (dev_id = 0; dev_id < OT_AI_DEV_MAX_NUM; dev_id++) {
    for (chn_id = 0; chn_id < OT_AI_MAX_CHN_NUM; chn_id++) {
      if (app_destroy_thread(dev_id, chn_id) != TD_SUCCESS) {
        printf("%s: app_destroy_thread(%d,%d) failed!\n", __FUNCTION__, dev_id,
               chn_id);
      }
    }
  }
  comm_audio_destroy_all_thread();
  comm_audio_exit();
}

static int app_getchar(td_void) {
  int c;

  if (g_app_exit == TD_TRUE) {
    app_exit_proc();
...
static FILE *app_open_file(ot_ai_chn ai_chn) {
  FILE *fd = TD_NULL;
  td_char asz_file_name[FILE_NAME_LEN] = {0};
  td_s32 ret;

  /* create file for save stream */
  ret = snprintf_s(asz_file_name, FILE_NAME_LEN, FILE_NAME_LEN - 1, "chn%d.pcm",
                   ai_chn);
  if (ret < 0) {
...
static td_s32 app_start(td_u32 ai_chn_cnt, ot_audio_dev ai_dev,
  ...
  for (i = 0; i < ai_chn_cnt; i++) {
    ...
    fd = app_open_file(ai_chn);
    if (fd == TD_NULL) {
      sample_dbg(TD_FAILURE);
      for (j = 0; j < i; j++) {
        app_destroy_thread(ai_dev, j);
      }
      return TD_FAILURE;
    }

    ret = app_create_thread(aio_attr, ai_dev, ai_chn, fd);
    if (ret != TD_SUCCESS) {
      sample_dbg(ret);
      fclose(fd);
      fd = TD_NULL;
...
static td_void app_stop(td_u32 ai_chn_cnt, ot_audio_dev ai_dev) {
  td_s32 ret;
  td_u32 i;
  ot_ai_chn ai_chn;
  ...
  for (i = 0; i < ai_chn_cnt; i++) {
    ai_chn = i;
    ret = app_destroy_thread(ai_dev, ai_chn);
    if (ret != TD_SUCCESS) {
      sample_dbg(ret);
    }
  }
...
static td_void app_init_params(ot_aio_attr *ai_attr, ot_audio_dev *ai_dev) {
...
td_s32 audio_dmic_ai_to_file(td_void) {
  td_s32 ret;
  ot_audio_dev ai_dev;
  td_u32 ai_chn_cnt;
  ot_aio_attr ai_attr = {};
  ...
  app_init_params(&ai_attr, &ai_dev);

  // start ai
  ai_chn_cnt = ai_attr.chn_cnt;
  ret = comm_audio_start_ai(ai_dev, ai_chn_cnt, &ai_attr, &ai_vqe_param, -1);
  if (ret != TD_SUCCESS) {
    sample_dbg(ret);
    return ret;
  }
  ...
  // set dmic gain
  ret = ss_mpi_ai_set_dmic_gain(ai_dev, APP_DMIC_GAIN);
  if (ret != TD_SUCCESS) {
    sample_dbg(ret);
    comm_audio_stop_ai(ai_dev, ai_chn_cnt, g_aio_resample, TD_TRUE);
    return ret;
  }

  // start record & write to file
  ret = app_start(ai_chn_cnt, ai_dev, &ai_attr);
...
static td_void app_handle_sig(td_s32 signo) {
  if (g_app_exit == TD_TRUE) {
    return;
  }
...
td_void app_sys_signal(void (*func)(int)) {

...
pa_channel_map *parameters_pa_channel_map_config(const char *fileConfig) {
  unsigned int nChannels =
      parameters_count(fileConfig, "raw.interface.channelmap");
  ...
  for (unsigned int iChannel = 0; iChannel < nChannels; ++iChannel) {
    ...
    if (((map->map[iChannel] = pa_channel_position_from_string(channel)) ==
  ...
  if (map->channels != parameters_lookup_int(fileConfig, "raw.nChannels")) {
    printf(
        "Configuration error: raw.interface.channelmap length does not match "
        "raw.nChannels\n");
    exit(EXIT_FAILURE);
  }
...
mod_resample_cfg *parameters_mod_resample_mics_config(const char *fileConfig) {
  ...
  if (tmpInt1 < tmpInt2) {
    printf(
        "general.samplerate.mu: This sample rate must be smaller or equal to "
        "raw.fS");
    exit(EXIT_FAILURE);
  }
...
msg_spectra_cfg *parameters_msg_spectra_mics_config(const char *fileConfig) {
  ...
  cfg->halfFrameSize =
      parameters_lookup_int(fileConfig, "general.size.frameSize") / 2 + 1;
...
mod_ssl_cfg *parameters_mod_ssl_config(const char *fileConfig) {
  ...
  for (iChannel = 0; iChannel < nChannels; iChannel++) {
    ...
    for (iSample = 0; iSample < 3; iSample++) {
      ...
      cfg->mics->mu[iChannel * 3 + iSample] =
          parameters_lookup_float(fileConfig, tmpLabel);
      free((void *)tmpLabel);
    }
    ...
    for (iSample = 0; iSample < 9; iSample++) {
      ...
      cfg->mics->sigma2[iChannel * 9 + iSample] =
          parameters_lookup_float(fileConfig, tmpLabel);
      free((void *)tmpLabel);
    }
    ...
    for (iSample = 0; iSample < 3; iSample++) {
      ...
      cfg->mics->direction[iChannel * 3 + iSample] =
          parameters_lookup_float(fileConfig, tmpLabel);
      free((void *)tmpLabel);
    }
  ...
  cfg->samplerate->mu =
      parameters_lookup_int(fileConfig, "general.samplerate.mu");
  cfg->samplerate->sigma2 =
      parameters_lookup_float(fileConfig, "general.samplerate.sigma2");
  ...
  cfg->soundspeed->mu =
      parameters_lookup_float(fileConfig, "general.speedofsound.mu");
  cfg->soundspeed->sigma2 =
      parameters_lookup_float(fileConfig, "general.speedofsound.sigma2");
  ...
  for (iFilter = 0; iFilter < nFilters; iFilter++) {
    ...
    for (iSample = 0; iSample < 3; iSample++) {
      tmpLabel = (char *)malloc(sizeof(char) * 1024);
      sprintf(tmpLabel, "general.spatialfilters.[%u].direction.[%u]", iFilter,
              iSample);
      cfg->spatialfilters->direction[iFilter * 3 + iSample] =
          parameters_lookup_float(fileConfig, tmpLabel);
      free((void *)tmpLabel);
    }
    ...
    cfg->spatialfilters->thetaAllPass[iFilter] =
        parameters_lookup_float(fileConfig, tmpLabel);
    free((void *)tmpLabel);
    ...
    cfg->spatialfilters->thetaNoPass[iFilter] =
        parameters_lookup_float(fileConfig, tmpLabel);
    free((void *)tmpLabel);
  }
...
snk_pots_cfg *parameters_snk_pots_ssl_config(const char *fileConfig) {
  ...
  if (strcmp(tmpStr1, "json") == 0) {
    cfg->format = format_construct_text_json();
  } else if (strcmp(tmpStr1, "binary") == 0) {
    cfg->format = format_construct_binary_float();
  } else if (strcmp(tmpStr1, "undefined") == 0) {
    cfg->format = format_construct_undefined();
  } else {
    printf("ssl.potential.format: Invalid format\n");
    exit(EXIT_FAILURE);
  }
  ...
  tmpStr1 =
      parameters_lookup_string(fileConfig, "ssl.potential.interface.type");
  ...
  if (strcmp(tmpStr1, "blackhole") == 0) {
    ...
    tmpStr2 =
        parameters_lookup_string(fileConfig, "ssl.potential.interface.path");
    cfg->interface = interface_construct_file(tmpStr2);
    free((void *)tmpStr2);

  } else if (strcmp(tmpStr1, "socket") == 0) {
    tmpStr2 =
        parameters_lookup_string(fileConfig, "ssl.potential.interface.ip");
...
mod_sst_cfg *parameters_mod_sst_config(const char *fileConfig) {
  ...
  cfg->nParticles =
      parameters_lookup_int(fileConfig, "sst.particle.nParticles");
  ...
  cfg->sigmaR_active =
      sqrtf(parameters_lookup_float(fileConfig, "sst.sigmaR2_active"));
  cfg->sigmaR_prob =
      sqrtf(parameters_lookup_float(fileConfig, "sst.sigmaR2_prob"));
  cfg->sigmaR_target =
      sqrtf(parameters_lookup_float(fileConfig, "sst.sigmaR2_target"));
  ...
  for (iGaussian = 0; iGaussian < nGaussians; iGaussian++) {
    ...
    cfg->active_gmm->array[iGaussian] =
        gaussian_1d_construct_weightmusigma(weight, mu, sigma);
  }
  ...
  for (iGaussian = 0; iGaussian < nGaussians; iGaussian++) {
    ...
    cfg->inactive_gmm->array[iGaussian] =
        gaussian_1d_construct_weightmusigma(weight, mu, sigma);
  }
  ...
  cfg->theta_inactive =
      parameters_lookup_float(fileConfig, "sst.theta_inactive");
  cfg->N_inactive =
      (unsigned int *)malloc(sizeof(unsigned int) * cfg->nTracksMax);
...
snk_tracks_cfg *parameters_snk_tracks_sst_config(const char *fileConfig) {
  ...
  if (strcmp(tmpStr1, "json") == 0) {
    ...
    printf("sst.tracked.format: Invalid format\n");
    exit(EXIT_FAILURE);
  }
  ...
  if (strcmp(tmpStr1, "blackhole") == 0) {
    ...
    tmpStr2 =
        parameters_lookup_string(fileConfig, "sst.tracked.interface.path");
    cfg->interface = interface_construct_file(tmpStr2);
    free((void *)tmpStr2);
...
mod_sss_cfg *parameters_mod_sss_config(const char *fileConfig) {
  ...
  cfg->pf_ms_alphaPmin =
      parameters_lookup_float(fileConfig, "sss.ms.alphaPmin");
  ...
  cfg->pf_ms_maxAbsenceProb =
      parameters_lookup_float(fileConfig, "sss.ms.maxAbsenceProb");
  ...
  cfg->pf_ms_winSizeLocal =
      parameters_lookup_int(fileConfig, "sss.ms.winSizeLocal");

  // +----------------------------------------------------------+
  // | winSizeGlobal                                            |
  // +----------------------------------------------------------+

  cfg->pf_ms_winSizeGlobal =
      parameters_lookup_int(fileConfig, "sss.ms.winSizeGlobal");

  // +----------------------------------------------------------+
  // | winSizeFrame                                             |
  // +----------------------------------------------------------+

  cfg->pf_ms_winSizeFrame =
      parameters_lookup_int(fileConfig, "sss.ms.winSizeFrame");
...
snk_hops_cfg *parameters_snk_hops_seps_vol_config(const char *fileConfig) {
  ...
  tmpStr1 =
      parameters_lookup_string(fileConfig, "sss.separated.interface.type");
  ...
  if (strcmp(tmpStr1, "blackhole") == 0) {
    ...
    tmpStr2 =
        parameters_lookup_string(fileConfig, "sss.separated.interface.path");
    cfg->interface = interface_construct_file(tmpStr2);
    free((void *)tmpStr2);

  } else if (strcmp(tmpStr1, "socket") == 0) {
    tmpStr2 =
        parameters_lookup_string(fileConfig, "sss.separated.interface.ip");
...
snk_hops_cfg *parameters_snk_hops_pfs_vol_config(const char *fileConfig) {
  ...
  tmpStr1 =
      parameters_lookup_string(fileConfig, "sss.postfiltered.interface.type");
  ...
  if (strcmp(tmpStr1, "blackhole") == 0) {
    ...
    tmpStr2 =
        parameters_lookup_string(fileConfig, "sss.postfiltered.interface.path");
    cfg->interface = interface_construct_file(tmpStr2);
    free((void *)tmpStr2);

  } else if (strcmp(tmpStr1, "socket") == 0) {
    tmpStr2 =
        parameters_lookup_string(fileConfig, "sss.postfiltered.interface.ip");
    tmpInt1 =
        parameters_lookup_int(fileConfig, "sss.postfiltered.interface.port");
...
snk_categories_cfg *parameters_snk_categories_config(const char *fileConfig) {
  ...
  if (strcmp(tmpStr1, "json") == 0) {
    ...
    printf("classify.category.format: Invalid format\n");
    exit(EXIT_FAILURE);
  }
  ...
  tmpStr1 =
      parameters_lookup_string(fileConfig, "classify.category.interface.type");

  if (strcmp(tmpStr1, "blackhole") == 0) {
    cfg->interface = interface_construct_blackhole();

  } else if (strcmp(tmpStr1, "file") == 0) {
    tmpStr2 = parameters_lookup_string(fileConfig,
                                       "classify.category.interface.path");
    cfg->interface = interface_construct_file(tmpStr2);
    free((void *)tmpStr2);

  } else if (strcmp(tmpStr1, "socket") == 0) {
    tmpStr2 =
        parameters_lookup_string(fileConfig, "classify.category.interface.ip");
    tmpInt1 =
        parameters_lookup_int(fileConfig, "classify.category.interface.port");

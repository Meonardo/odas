...
int threads_single_process(objects* objs, profiler* prf) {
  ...
  prf->duration += ((float)(objs->src_hops_mics_object->hopSize)) /
                   ((float)(objs->src_hops_mics_object->fS));
  ...
  prf->con_hops_mics_raw_prf +=
      (float)(((double)(end - begin)) / CLOCKS_PER_SEC);
  ...
  // Loop through section II as long as there are frames to process from the
  // resample module
  while (1) {
    // +------------------------------------------------------+
    // | Resample                                             |

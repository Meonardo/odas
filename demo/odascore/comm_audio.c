...
void *comm_audio_ai_proc(void *parg) {
  td_s32 ret;
  td_s32 ai_fd;
...
void *comm_audio_aenc_proc(void *parg) {
  td_s32 ret;
  td_s32 aenc_fd;
...
void *comm_audio_adec_proc(void *parg) {
  td_s32 ret;
...
/* set ao volume */
void *comm_audio_ao_vol_proc(void *parg) {
  td_s32 ret;
  td_s32 volume;
  ot_audio_fade fade;
...
td_s32 comm_audio_create_thread_ai_ao(ot_audio_dev ai_dev,
  ...
  pthread_create(&ai->ai_pid, 0, comm_audio_ai_proc, ai);
...
td_s32 comm_audio_create_thread_aenc_adec(ot_aenc_chn ae_chn,
  ...
  aenc->fd = aenc_fd;
  aenc->start = TD_TRUE;
  pthread_create(&aenc->aenc_pid, 0, comm_audio_aenc_proc, aenc);
...
td_s32 comm_audio_create_thread_file_adec(ot_adec_chn ad_chn,
  ...
  adec->fd = adec_fd;
  adec->start = TD_TRUE;
  pthread_create(&adec->ad_pid, 0, comm_audio_adec_proc, adec);
...
td_s32 comm_audio_create_thread_ao_vol_ctrl(ot_audio_dev ao_dev) {
  ...
  pthread_create(&ao_ctl->ao_pid, 0, comm_audio_ao_vol_proc, ao_ctl);
...
static td_s32 comm_audio_start_ai_vqe(
    ...
    const comm_ai_vqe_param *ai_vqe_param, ot_audio_dev ao_dev_id) {
  td_s32 ret;
...
td_s32 comm_audio_start_ai(ot_audio_dev ai_dev_id, td_u32 ai_chn_cnt,
  ...
  for (i = 0; i < (td_s32)chn_cnt; i++) {
    ...
    ret = comm_audio_start_ai_vqe(ai_dev_id, i, ai_vqe_param, ao_dev_id);
    if (ret != TD_SUCCESS) {
      return ret;
    }
  }
...
td_s32 comm_audio_destroy_all_thread(void) {
  td_u32 dev_id, chn_id;

  for (dev_id = 0; dev_id < OT_AI_DEV_MAX_NUM; dev_id++) {
    for (chn_id = 0; chn_id < OT_AI_MAX_CHN_NUM; chn_id++) {
      if (comm_audio_destroy_thread_ai(dev_id, chn_id) != TD_SUCCESS) {
        printf("%s: comm_audio_destroy_thread_ai(%d,%d) failed!\n",
  ...
  for (chn_id = 0; chn_id < OT_AENC_MAX_CHN_NUM; chn_id++) {
    if (comm_audio_destroy_thread_aenc_adec(chn_id) != TD_SUCCESS) {
      printf("%s: comm_audio_destroy_thread_aenc_adec(%d) failed!\n",
             __FUNCTION__, chn_id);
      return TD_FAILURE;
    }
  }
  ...
  for (chn_id = 0; chn_id < OT_ADEC_MAX_CHN_NUM; chn_id++) {
    if (comm_audio_destroy_thread_file_adec(chn_id) != TD_SUCCESS) {
      printf("%s: comm_audio_destroy_thread_file_adec(%d) failed!\n",
             __FUNCTION__, chn_id);
      return TD_FAILURE;
    }
  }
  ...
  for (chn_id = 0; chn_id < OT_AO_DEV_MAX_NUM; chn_id++) {
    if (comm_audio_destroy_thread_ao_vol_ctrl(chn_id) != TD_SUCCESS) {
      printf("%s: comm_audio_destroy_thread_ao_vol_ctrl(%d) failed!\n",
             __FUNCTION__, chn_id);
      return TD_FAILURE;
    }
  }

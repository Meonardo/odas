...
static td_char *g_ai_vqe_str[SAMPLE_AUDIO_VQE_TYPE_MAX] = {"none", "record",
                                                           "talk", "talkv2"};
...
typedef td_void *(*ot_resample_create_callback)(td_s32 in_rate, td_s32 out_rate,
                                                td_s32 chn_num);
typedef td_s32 (*ot_resample_process_callback)(td_void *instance,
                                               const td_s16 *in_buf,
                                               td_s32 in_samples,
                                               td_s16 *out_buf);
typedef td_void (*ot_resample_destroy_callback)(td_void *instance);
typedef td_s32 (*ot_resample_get_max_output_num_callback)(td_void *instance,
                                                          td_s32 in_samples);
...
static ot_sample_ai_to_ext_res
    g_sample_ai_ext_res[OT_AI_DEV_MAX_NUM * OT_AI_MAX_CHN_NUM];
static ot_sample_res_fun g_sample_res_fun = {0};

static td_void sample_audio_ai_to_res_proc(ot_sample_ai_to_ext_res *ai_ctrl,
                                           ot_audio_frame *frame, td_s16 *cache,
                                           td_u16 *cache_count,
                                           td_u16 cache_sample) {
  ...
  if (mulit != 1) {
    if (*cache_count != 0) {
      ...
      ret = memcpy_s(cache + (*cache_count),
                     (cache_sample - *cache_count) * sizeof(td_s16),
                     frame->virt_addr[0],
                     (mulit - *cache_count) * sizeof(td_s16));
      if (ret != EOK) {
        printf("%s: memcpy_s failed, ret:0x%x.\n", __FUNCTION__, ret);
        return;
      }
      out_sample = g_sample_res_fun.resample_process_callback(
          g_res_handle, cache, mulit, g_res_out_buf);
    ...
    out_sample = g_sample_res_fun.resample_process_callback(
        g_res_handle,
        (td_s16 *)(frame->virt_addr[0] +
                   (mulit - *cache_count) * sizeof(td_s16)),
        proc_frame, g_res_out_buf);
    (td_void) fwrite(g_res_out_buf, 2, out_sample, ai_ctrl->fd); /* 2:byte */

    cur_sample = proc_frame + (mulit - *cache_count);
    if ((ai_ctrl->per_frame - cur_sample) != 0) {
      ret = memcpy_s(cache, cache_sample * sizeof(td_s16),
                     frame->virt_addr[0] + cur_sample * sizeof(td_s16),
                     (ai_ctrl->per_frame - cur_sample) * sizeof(td_s16));
      if (ret != EOK) {
        printf("%s: memcpy_s failed, ret:0x%x.\n", __FUNCTION__, ret);
        return;
      }
    ...
    *cache_count = ai_ctrl->per_frame - cur_sample;
    ...
    out_sample = g_sample_res_fun.resample_process_callback(
        g_res_handle, (td_s16 *)frame->virt_addr[0], ai_ctrl->per_frame,
        g_res_out_buf);
    (td_void) fwrite(g_res_out_buf, 2, out_sample, ai_ctrl->fd); /* 2:byte */
  }
...
static td_s32 sample_audio_ai_to_res_init(ot_sample_ai_to_ext_res *ai_ctrl) {
  ...
  sample_res_check_null_ptr(
      g_sample_res_fun.resample_get_max_output_num_callback);
  ...
  g_res_handle = g_sample_res_fun.resample_create_callback(
      ai_ctrl->in_sample, ai_ctrl->out_sample, 1);
  if (g_res_handle == TD_NULL) {
    printf("%s: ot_resampler_create failed, ret=0x%x\n", __FUNCTION__, ret);
    return TD_FAILURE;
  }

  out_max_num = g_sample_res_fun.resample_get_max_output_num_callback(
      g_res_handle, ai_ctrl->per_frame);
  if (out_max_num <= 0) {
  ...
  g_res_out_buf =
      malloc(out_max_num * sizeof(td_s16) + 2); /* 2:Reserved space */
...
void *sample_comm_audio_ai_ext_res_proc(void *parg) {
  ...
  while (ai_ctrl->start) {
    ...
    if (FD_ISSET(ai_fd, &read_fds)) {
      ...
      sample_audio_ai_to_res_proc(ai_ctrl, &frame, cache, &cache_count,
                                  sizeof(cache) / sizeof(td_s16));
...
td_s32 sample_comm_audio_create_thread_ai_ext_res(
    ot_aio_attr *aio_attr, ot_audio_dev ai_dev, ot_ai_chn ai_chn,
    ot_audio_sample_rate out_sample_rate, FILE *res_fd) {
  ...
  pthread_create(&ai_to_ext_res->ai_pid, 0, sample_comm_audio_ai_ext_res_proc,
                 ai_to_ext_res);
...
/* function : Create the thread to get frame from ai and write to file */
td_s32 sample_comm_audio_create_thread_ai_to_file(ot_aio_attr *aio_attr,
                                                  ot_audio_dev ai_dev,
                                                  ot_ai_chn ai_chn,
                                                  FILE *res_fd) {
  ot_sample_ai_to_ext_res *ai_to_ext_res = TD_NULL;
...
/* function : Destroy the thread to get frame from ai and send to extern
 * resamler */
td_s32 sample_comm_audio_destroy_thread_ai_ext_res(ot_audio_dev ai_dev,
                                                   ot_ai_chn ai_chn) {
  ot_sample_ai_to_ext_res *ai_to_ext_res = TD_NULL;
...
static td_void sample_audio_deinit_ext_res_fun(td_void) {
  ...
  ret = memset_s(&g_sample_res_fun, sizeof(ot_sample_res_fun), 0,
                 sizeof(ot_sample_res_fun));
  if (ret != EOK) {
...
static td_s32 sample_audio_init_ext_res_fun(td_void) {
  ...
  sample_res_fun.resample_get_max_output_num_callback =
  ...
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "load resample lib fail!\n");
    return TD_FAILURE;
  }

  ret = ot_audio_dlsym((td_void **)&(sample_res_fun.resample_create_callback),
                       sample_res_fun.lib_handle, "ot_resample_create");
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "find symbol error!\n");
    goto dysym_fail;
  ...
  ret = ot_audio_dlsym((td_void **)&(sample_res_fun.resample_process_callback),
                       sample_res_fun.lib_handle, "ot_resample_process");
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "find symbol error!\n");
    goto dysym_fail;
  ...
  ret = ot_audio_dlsym((td_void **)&(sample_res_fun.resample_destroy_callback),
                       sample_res_fun.lib_handle, "ot_resample_destroy");
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "find symbol error!\n");
    goto dysym_fail;
  ...
  ret = ot_audio_dlsym(
      (td_void **)&(sample_res_fun.resample_get_max_output_num_callback),
      sample_res_fun.lib_handle, "ot_resample_get_max_output_num");
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "find symbol error!\n");
    goto dysym_fail;
  ...
  (td_void) memcpy_s(&g_sample_res_fun, sizeof(g_sample_res_fun),
                     &sample_res_fun, sizeof(sample_res_fun));
...
static td_void sample_audio_exit_proc(td_void) {
  ...
  for (dev_id = 0; dev_id < OT_AI_DEV_MAX_NUM; dev_id++) {
    for (chn_id = 0; chn_id < OT_AI_MAX_CHN_NUM; chn_id++) {
      if (sample_comm_audio_destroy_thread_ai_ext_res(dev_id, chn_id) !=
          TD_SUCCESS) {
        printf(
            "%s: sample_comm_audio_destroy_thread_ai_ext_res(%d,%d) failed!\n",
...
static FILE *sample_audio_open_aenc_file(ot_aenc_chn ae_chn,
...
#ifdef __LITEOS__
  ret =
      snprintf_s(asz_file_name, FILE_NAME_LEN, FILE_NAME_LEN - 1,
  ...
  if (ret < 0) {
    printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__,
           "get aenc file name failed");
    return TD_NULL;
  }

  if (asz_file_name[0] == '\0') {
    printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__,
           "aenc file name is NULL");
    return TD_NULL;
  }

  if (strlen(asz_file_name) > (FILE_NAME_LEN - 1)) {
    printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__,
           "aenc file name extra long");
    return TD_NULL;
  }
...
static FILE *sample_audio_open_adec_file(ot_adec_chn ad_chn,
                                         ot_payload_type type) {
  FILE *fd = TD_NULL;
  td_char asz_file_name[FILE_NAME_LEN] = {0};
  td_s32 ret;
  ...
  if (ret < 0) {
    printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__,
           "get adec file name failed");
    return TD_NULL;
  }

  if (asz_file_name[0] == '\0') {
    printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__,
           "adec file name is NULL");
    return TD_NULL;
  }

  if (strlen(asz_file_name) > (FILE_NAME_LEN - 1)) {
    printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__,
           "adec file name extra long");
    return TD_NULL;
  }

  if (realpath(asz_file_name, path) == TD_NULL) {
    printf("[func]:%s [line]:%d [info]:%s\n", __FUNCTION__, __LINE__,
           "adec file name realpath fail");
    return TD_NULL;
  }
...
static td_void sample_audio_set_ai_vqe_param(
    sample_comm_ai_vqe_param *ai_vqe_param,
    ot_audio_sample_rate out_sample_rate, td_bool resample_en,
    td_void *ai_vqe_attr, sample_audio_vqe_type ai_vqe_type) {
...
static td_void sample_audio_adec_ao_init_param(ot_aio_attr *aio_attr,
                                               ot_audio_dev *ao_dev) {
...
td_void sample_audio_adec_ao_inner(ot_audio_dev ao_dev, ot_ao_chn ao_chn,
                                   ot_adec_chn ad_chn) {
  td_s32 ret;
  FILE *fd = TD_NULL;
...
td_s32 sample_audio_adec_ao(td_void) {
  ...
  ret = sample_comm_audio_start_ao(ao_dev, ao_chn_cnt, &aio_attr,
...
static td_void sample_audio_ai_aenc_init_param(ot_aio_attr *aio_attr,
                                               ot_audio_dev *ai_dev,
                                               ot_audio_dev *ao_dev) {
  aio_attr->sample_rate = OT_AUDIO_SAMPLE_RATE_48000;
  aio_attr->bit_width = OT_AUDIO_BIT_WIDTH_16;
...
static td_s32 sample_audio_aenc_bind_ai(ot_audio_dev ai_dev,
                                        td_u32 aenc_chn_cnt) {
  td_s32 ret;
  td_u32 i, j;
  ot_ai_chn ai_chn;
...
static td_void sample_audio_aenc_unbind_ai(ot_audio_dev ai_dev,
                                           td_u32 aenc_chn_cnt) {
  td_s32 ret;
  td_u32 i;
  ot_ai_chn ai_chn;
...
static td_s32 sample_audio_ao_bind_adec(td_u32 adec_chn_cnt,
                                        ot_audio_dev ao_dev) {
  td_s32 ret;
  td_u32 i, j;
  ot_ao_chn ao_chn;
...
static td_void sample_audio_ao_unbind_adec(td_u32 adec_chn_cnt,
                                           ot_audio_dev ao_dev) {
  td_s32 ret;
  td_u32 i;
  ot_ao_chn ao_chn;
...
static td_void sample_audio_ai_aenc_inner(td_bool send_adec,
                                          ot_aio_attr *aio_attr,
                                          ot_audio_dev ao_dev) {
  td_s32 ret;
  td_u32 i;
...
td_s32 sample_audio_ai_aenc(td_void) {
  ...
  sample_audio_set_ai_vqe_param(&ai_vqe_param, g_out_sample_rate,
                                g_aio_resample, TD_NULL, 0);
  ret = sample_comm_audio_start_ai(ai_dev, ai_chn_cnt, &aio_attr, &ai_vqe_param,
                                   -1);
  if (ret != TD_SUCCESS) {
    sample_dbg(ret);
    goto ai_aenc_err6;
...
static td_void sample_audio_ai_to_ext_resample_init_param(
    ot_aio_attr *aio_attr, ot_audio_dev *ai_dev) {
...
static td_s32 sample_audio_start_ai_resample(td_u32 ai_chn_cnt,
                                             ot_audio_dev ai_dev,
                                             ot_aio_attr *aio_attr) {
  td_s32 ret;
  td_u32 i, j;
  ...
  for (i = 0; i < ai_chn_cnt; i++) {
    ...
    ret = sample_comm_audio_create_thread_ai_ext_res(aio_attr, ai_dev, ai_chn,
                                                     g_out_sample_rate, fd);
    if (ret != TD_SUCCESS) {
      sample_dbg(ret);
      fclose(fd);
      fd = TD_NULL;
...
static td_s32 sample_audio_start_ai_to_file(td_u32 ai_chn_cnt,
  ...
  for (i = 0; i < ai_chn_cnt; i++) {
    ...
    ret = sample_comm_audio_create_thread_ai_to_file(aio_attr, ai_dev, ai_chn,
                                                     fd);
    if (ret != TD_SUCCESS) {
      sample_dbg(ret);
      fclose(fd);
...
static td_void sample_audio_stop_ai_resample(td_u32 ai_chn_cnt,
                                             ot_audio_dev ai_dev) {
  td_s32 ret;
  td_u32 i;
  ot_ai_chn ai_chn;
...
static td_void sample_audio_dmic_ai_to_file_init_params(ot_aio_attr *ai_attr,
  ...
  ai_attr->point_num_per_frame = OT_AACLC_SAMPLES_PER_FRAME;
...
td_s32 sample_audio_dmic_ai_to_file(td_void) {
  ...
  // config audio codec
  ret = sample_comm_audio_cfg_acodec(&ai_attr);
  if (ret != TD_SUCCESS) {
    sample_dbg(ret);
    sample_comm_audio_stop_ai(ai_dev, ai_chn_cnt, g_aio_resample, TD_TRUE);
    return ret;
...
static td_void sample_audio_ai_ao_init_param(ot_aio_attr *aio_attr,
                                             ot_audio_dev *ai_dev,
                                             ot_audio_dev *ao_dev) {
  aio_attr->sample_rate = OT_AUDIO_SAMPLE_RATE_48000;
  aio_attr->bit_width = OT_AUDIO_BIT_WIDTH_16;
...
static td_void sample_audio_ai_to_ao_sys_chn_init_param(ot_aio_attr *aio_attr,
                                                        ot_audio_dev *ai_dev,
                                                        ot_audio_dev *ao_dev) {
  aio_attr->sample_rate = OT_AUDIO_SAMPLE_RATE_48000;
  aio_attr->bit_width = OT_AUDIO_BIT_WIDTH_16;
...
static td_void sample_audio_ai_to_ao_sys_chn_inner(ot_audio_dev ai_dev,
                                                   ot_ai_chn ai_chn,
                                                   ot_audio_dev ao_dev) {
  td_s32 ret;

  /* bind AI to AO channel */
  ret = sample_comm_audio_create_thread_ai_ao(ai_dev, ai_chn, ao_dev,
                                              OT_AO_SYS_CHN_ID);
  if (ret != TD_SUCCESS) {
    sample_dbg(ret);
    return;
  }
  printf("ai(%d,%d) bind to ao(%d,%d) ok\n", ai_dev, ai_chn, ao_dev,
         OT_AO_SYS_CHN_ID);
...
static td_void ai_init_record_vqe_param(
    ot_ai_record_vqe_cfg *ai_vqe_record_attr) {
  ...
  ai_vqe_record_attr->open_mask =
      OT_AI_RECORDVQE_MASK_DRC | OT_AI_RECORDVQE_MASK_HDR |
      OT_AI_RECORDVQE_MASK_HPF | OT_AI_RECORDVQE_MASK_RNR;
}
...
static td_void ai_init_talk_vqe_param(ot_ai_talk_vqe_cfg *ai_vqe_talk_attr) {
  ...
  ai_vqe_talk_attr->open_mask =
      OT_AI_TALKVQE_MASK_AGC | OT_AI_TALKVQE_MASK_ANR | OT_AI_TALKVQE_MASK_HPF;
}

static td_void ai_init_talk_vqe_v2_param(
    ot_ai_talk_vqe_v2_cfg *ai_vqe_talk_v2_attr) {
  ...
  ai_vqe_talk_v2_attr->open_mask = OT_AI_TALKVQEV2_MASK_AGC |
                                   OT_AI_TALKVQEV2_MASK_NR |
                                   OT_AI_TALKVQEV2_MASK_WNR;
}

static td_void sample_audio_ai_vqe_process_ao_init_param(
    ot_aio_attr *aio_attr, ot_audio_dev *ai_dev, ot_audio_dev *ao_dev,
    td_void **ai_vqe_attr) {
  aio_attr->sample_rate = OT_AUDIO_SAMPLE_RATE_48000;
  aio_attr->bit_width = OT_AUDIO_BIT_WIDTH_16;
  aio_attr->work_mode = OT_AIO_MODE_I2S_MASTER;
...
static td_s32 sample_audio_ao_bind_ai_multi_chn(ot_audio_dev ai_dev,
                                                const ot_aio_attr *ai_attr,
                                                ot_audio_dev ao_dev) {
  td_s32 ret;
  td_u32 i, j;
...
static td_void sample_audio_ao_unbind_ai_multi_chn(ot_audio_dev ai_dev,
                                                   const ot_aio_attr *ai_attr) {
  td_s32 ret;
  td_u32 i;
  td_u32 ai_chn_cnt = ai_attr->chn_cnt;
...
td_s32 sample_audio_ai_vqe_process_ao(td_void) {
  ...
  sample_audio_ai_vqe_process_ao_init_param(&aio_attr, &ai_dev, &ao_dev,
                                            &ai_vqe_attr);
  ...
  ai_chn_cnt = aio_attr.chn_cnt;
  sample_audio_set_ai_vqe_param(&ai_vqe_param, g_out_sample_rate,
                                g_aio_resample, ai_vqe_attr, g_ai_vqe_type);
  ret = sample_comm_audio_start_ai(ai_dev, ai_chn_cnt, &aio_attr, &ai_vqe_param,
                                   ao_dev);
  if (ret != TD_SUCCESS) {
    sample_dbg(ret);
...
static td_void sample_audio_dimc_ai_ao_init_param(ot_aio_attr *ai_attr,
                                                  ot_audio_dev *ai_dev,
                                                  ot_aio_attr *ao_attr,
                                                  ot_audio_dev *ao_dev) {
  ai_attr->sample_rate = OT_AUDIO_SAMPLE_RATE_48000;
  ai_attr->bit_width = OT_AUDIO_BIT_WIDTH_16;
  ai_attr->work_mode = OT_AIO_MODE_I2S_MASTER;
  ai_attr->snd_mode = OT_AUDIO_SOUND_MODE_STEREO;
  ai_attr->expand_flag = 0;
  ai_attr->frame_num = SAMPLE_AUDIO_AI_USER_FRAME_DEPTH;
  ai_attr->point_num_per_frame = OT_AACLC_SAMPLES_PER_FRAME;
  ai_attr->chn_cnt = 2; /* 2:chn num */

  ao_attr->sample_rate = OT_AUDIO_SAMPLE_RATE_48000;
  ao_attr->bit_width = OT_AUDIO_BIT_WIDTH_16;
  ao_attr->work_mode = OT_AIO_MODE_I2S_MASTER;
  ao_attr->snd_mode = OT_AUDIO_SOUND_MODE_STEREO;
  ao_attr->expand_flag = 0;
  ao_attr->frame_num = SAMPLE_AUDIO_AI_USER_FRAME_DEPTH;
  ao_attr->point_num_per_frame = OT_AACLC_SAMPLES_PER_FRAME;
  ao_attr->chn_cnt = 2; /* 2:chn num */
  ...
  *ao_dev = SAMPLE_AUDIO_INNER_AO_DEV;
  ...
  ai_attr->i2s_type = OT_AIO_I2STYPE_DMIC;
#ifdef OT_ACODEC_TYPE_INNER
  ao_attr->i2s_type = OT_AIO_I2STYPE_INNERCODEC;
#else
  ao_attr->i2s_type = OT_AIO_I2STYPE_EXTERN;
#endif
}

static td_void sample_audio_dmic_ai_ao_inner(ot_audio_dev ai_dev,
                                             ot_ai_chn ai_chn,
                                             ot_audio_dev ao_dev,
                                             ot_ao_chn ao_chn) {
  td_s32 ret;
...
/* function : ai(dmic) -> ao */
td_s32 sample_audio_dmic_ai_ao(td_void) {
  td_s32 ret;
  ot_audio_dev ai_dev, ao_dev;
  ot_aio_attr ai_attr, ao_attr;
  td_u32 ai_chn_cnt, ao_chn_cnt;
  sample_comm_ai_vqe_param ai_vqe_param = {0};
  const ot_ai_chn ai_chn = 0;
  const ot_ao_chn ao_chn = 0;

  sample_audio_dimc_ai_ao_init_param(&ai_attr, &ai_dev, &ao_attr, &ao_dev);
  /* step 1: start ai */
  ai_chn_cnt = ai_attr.chn_cnt;
  ret = sample_comm_audio_start_ai(ai_dev, ai_chn_cnt, &ai_attr, &ai_vqe_param,
                                   -1);
  ...
  ret = sample_comm_audio_start_ao(ao_dev, ao_chn_cnt, &ao_attr,
                                   g_in_sample_rate, g_aio_resample);
  if (ret != TD_SUCCESS) {
    sample_dbg(ret);
    goto dmic_err1;
  ...
  /* step 4: config audio codec */
  ret = sample_comm_audio_cfg_acodec(&ao_attr);
  if (ret != TD_SUCCESS) {
    sample_dbg(ret);
    goto dmic_err0;
  }

  sample_audio_dmic_ai_ao_inner(ai_dev, ai_chn, ao_dev, ao_chn);
...
td_void sample_audio_usage(td_void) {
  printf("\n\nusage:./sample_audio <index> [vqe_type]\n");
  ...
  printf(
      "\t    vqe_type: 0: none, 1: record, 2: talk, 3: talkv2 (default: 1)\n");
  printf("\t4:  start AI(DMIC) to AO loop\n");

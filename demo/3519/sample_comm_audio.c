...
#define AUDIO_ADPCM_TYPE \
  OT_ADPCM_TYPE_DVI4               /* ADPCM_TYPE_IMA, ADPCM_TYPE_DVI4 */
...
#ifdef OT_ACODEC_TYPE_ES8388
td_s32 sample_es8388_get_clk_ratio(ot_audio_sample_rate sample_rate,
                                   td_u32 *fsclk_ratio, td_u32 *bclk_ratio) {
  switch (sample_rate) {
...
td_s32 sample_es8388_set_input_ctrl(int codec_fd) {
  ...
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "es83888 set left in power fail.");
    return ret;
  }
  ret = ioctl(codec_fd, OT_ES8388_SET_RIGHT_INPUT_POWER, &audio_ctrl);
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "es83888 set right in power fail.");
    return ret;
  }
  ...
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "es83888 set left ADC power fail.");
    return ret;
  }
  ret = ioctl(codec_fd, OT_ES8388_SET_RIGHT_ADC_POWER, &audio_ctrl);
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "es83888 set right ADC power fail.");
    return ret;
  }
...
td_s32 sample_es8388_set_clk_mode(int codec_fd, ot_aio_mode work_mode,
  ...
  /* set master clk */
  if (work_mode == OT_AIO_MODE_I2S_SLAVE ||
      work_mode == OT_AIO_MODE_PCM_SLAVE_STD ||
    ...
    if (ret != TD_SUCCESS) {
      printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
             "es83888 set ADC LRCLK ratio fail.");
      return ret;
    }
    ...
    if (ret != TD_SUCCESS) {
      printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
             "es83888 set DAC LRCLK ratio fail.");
      return ret;
    }
  }
...
td_s32 sample_es8388_set_clk_polarity(int codec_fd, ot_aio_mode work_mode) {
  ...
  if (work_mode == OT_AIO_MODE_PCM_MASTER_STD ||
      work_mode == OT_AIO_MODE_PCM_SLAVE_STD) {
    /* PCM offset is 1 */
    pcm_offset = ES8388_PCM_OFFSET_2ND;
    clk_dir = ES8388_BCLK_DIR_INVERT;
  } else if (work_mode == OT_AIO_MODE_PCM_MASTER_NON_STD ||
             work_mode == OT_AIO_MODE_PCM_SLAVE_NON_STD) {
    /* PCM offset is 0 */
  ...
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "es83888 set ADC polarity or offset fail.");
    return ret;
  }
  ret = ioctl(codec_fd, OT_ES8388_SET_DAC_POLARITY_AND_OFFSET, &audio_ctrl);
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "es83888 set DAC polarity or offset fail.");
    return ret;
  }
...
td_s32 sample_es8388_set_clk_ctrl(int codec_fd, ot_aio_mode work_mode,
                                  ot_audio_sample_rate sample_rate) {
  td_s32 ret;
...
td_s32 sample_es8388_set_data_ctrl(int codec_fd, ot_aio_mode work_mode,
  ...
  /* set data format */
  if (work_mode == OT_AIO_MODE_I2S_MASTER ||
      work_mode == OT_AIO_MODE_I2S_SLAVE) {
    /* i2s */
    format_ctrl = ES8388_DATA_FORMAT_I2S;
  } else if (work_mode == OT_AIO_MODE_PCM_MASTER_STD ||
             work_mode == OT_AIO_MODE_PCM_SLAVE_STD ||
             work_mode == OT_AIO_MODE_PCM_MASTER_NON_STD ||
             work_mode == OT_AIO_MODE_PCM_SLAVE_NON_STD) {
    /* pcm */
...
td_s32 sample_es8388_set_output_ctrl(int codec_fd) {
  ...
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "es83888 set left DAC power fail.");
    return ret;
  }
  ret = ioctl(codec_fd, OT_ES8388_SET_RIGHT_DAC_POWER, &audio_ctrl);
  if (ret != TD_SUCCESS) {
    printf("[Func]:%s [Line]:%d [Info]:%s\n", __FUNCTION__, __LINE__,
           "es83888 set right DAC power fail.");
    return ret;
  }
...
td_s32 sample_es8388_cfg_audio(ot_aio_mode work_mode,
                               ot_audio_sample_rate sample_rate,
                               ot_audio_bit_width bit_width) {
  td_s32 ret;
  int es8388_fd = -1;
...
#ifdef OT_ACODEC_TYPE_INNER
static td_s32 inner_codec_get_i2s_fs(ot_audio_sample_rate sample_rate,
                                     ot_acodec_fs *i2s_fs) {
...
td_s32 sample_inner_codec_cfg_audio(ot_audio_sample_rate sample_rate) {
  ...
  /* refer to hardware, demo board is pseudo-differential (IN_D), socket board
   * is single-ended (IN1) */
  ...
  /*
   * The input volume range is [-78, 80]. Both the analog gain and digital gain
   * are adjusted. A larger value indicates higher volume. For example, the
   * value 80 indicates the maximum volume of 80 dB, and the value -78 indicates
   * the minimum volume (muted status). The volume adjustment takes effect
   * simultaneously in the audio-left and audio-right channels. The recommended
   * volume range is [20, 50]. Within this range, the noises are lowest because
   * only the analog gain is adjusted, and the voice quality can be guaranteed.
   */
...
#ifdef OT_ACODEC_TYPE_ES8388
  td_s32 ret = sample_es8388_cfg_audio(
      aio_attr->work_mode, aio_attr->sample_rate, aio_attr->bit_width);
  if (ret != TD_SUCCESS) {
...
static td_s32 sample_comm_audio_start_ai_vqe(
    ot_audio_dev ai_dev_id, ot_ai_chn ai_chn,
  ...
  if (ai_vqe_param->ai_vqe_attr != NULL) {
    ...
    if (ret != TD_SUCCESS) {
      printf("%s: set_ai_vqe%d(%d,%d) failed with %#x\n", __FUNCTION__,
             ai_vqe_param->ai_vqe_type, ai_dev_id, ai_chn, ret);
      return ret;
    }
...
static td_void aenc_attr_opus_get_param(const ot_aio_attr *aio_attr,
...
static td_void aenc_attr_mp3_get_param(const ot_aio_attr *aio_attr,
...
static td_void aenc_attr_aac_get_param(const ot_aio_attr *aio_attr,
...
td_s32 sample_comm_audio_start_aenc(td_u32 aenc_chn_cnt,
  ...
  } else if ((aenc_attr.type == OT_PT_G711A) ||
...
td_s32 sample_comm_audio_start_adec(td_u32 adec_chn_cnt,
  ...
  } else if ((adec_attr.type == OT_PT_G711A) ||
             (adec_attr.type == OT_PT_G711U)) {

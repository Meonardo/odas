...
*************************************************/
static int get_write_space(unsigned char *write_point,
                           unsigned char *read_point, int buffer_size,
                           int last_is_write) {
  int bytes = 0;
...
*************************************************/
static size_t get_read_space(unsigned char *write_point,
                             unsigned char *read_point, int buffer_size,
                             int last_is_write) {
  int bytes = 0;
...
*************************************************/
static int write_to_buffer(unsigned char *current_pointer, unsigned char *data,
                           int bytes, unsigned char *start_addr,
                           int total_size) {
...
Return: 0 for success
*************************************************/
static int read_from_buffer(unsigned char *current_pointer,
...
*************************************************/
size_t ring_buffer_write(struct ring_buffer *rbuffer, unsigned char *data,
                         size_t bytes, int cover) {
  ...
  write_space =
      get_write_space(buf->wr, buf->rd, buf->size, buf->last_is_write);
  ...
  if (written_bytes) buf->last_is_write = 1;
...
*************************************************/
size_t ring_buffer_read(struct ring_buffer *rbuffer, unsigned char *buffer,
                        size_t bytes) {
  struct ring_buffer *buf = rbuffer;
  ...
  readable_space =
      get_read_space(buf->wr, buf->rd, buf->size, buf->last_is_write);
  ...
  if (read_bytes) buf->last_is_write = 0;
  pthread_mutex_unlock(&buf->lock);
...
int get_buffer_read_space(struct ring_buffer *rbuffer) {
  ...
  pthread_mutex_lock(&rbuffer->lock);
  if (rbuffer->start_addr == NULL || rbuffer->wr == NULL ||
      rbuffer->rd == NULL || rbuffer->size == 0) {
    printf("Buffer malloc fail!\n");
    pthread_mutex_unlock(&rbuffer->lock);
    return -1;
  }

  read_space = get_read_space(rbuffer->wr, rbuffer->rd, rbuffer->size,
                              rbuffer->last_is_write);
  pthread_mutex_unlock(&rbuffer->lock);
...
int get_buffer_write_space(struct ring_buffer *rbuffer) {
  ...
  pthread_mutex_lock(&rbuffer->lock);
  if (rbuffer->start_addr == NULL || rbuffer->wr == NULL ||
      rbuffer->wr == NULL || rbuffer->size == 0) {
    printf("Buffer malloc fail!\n");
    pthread_mutex_unlock(&rbuffer->lock);
    return -1;
  }

  write_space = get_write_space(rbuffer->wr, rbuffer->rd, rbuffer->size,
                                rbuffer->last_is_write);
  pthread_mutex_unlock(&rbuffer->lock);
...
void ring_buffer_dump(struct ring_buffer *rbuffer) {
  printf("buffer_avail:%d, buffer_space:%d\n", get_buffer_read_space(rbuffer),
         get_buffer_write_space(rbuffer));
}
#ifdef __cplusplus
}
#endif
